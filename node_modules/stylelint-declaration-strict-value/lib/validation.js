...
function validProperties(actual) {
  return isNumberOrString(actual) || Array.isArray(actual) && actual.every(function (item) {
    return isNumberOrString(item);
  });
}
...
function validOptions(actual) {
  if ((typeof actual === 'undefined' ? 'undefined' : _typeof(actual)) !== 'object') return false;

  var allowedKeys = Object.keys(_defaults2.default);
  if (!Object.keys(actual).every(function (key) {
    return allowedKeys.indexOf(key) > -1;
  })) return false;

  if ('ignoreVariables' in actual && typeof actual.ignoreVariables !== 'boolean' && actual.ignoreVariables !== null) return false;

  if ('ignoreFunctions' in actual && typeof actual.ignoreFunctions !== 'boolean' && actual.ignoreFunctions !== null) return false;

  if ('severity' in actual && typeof actual.severity !== 'string' && actual.severity !== null) return false;

  if ('ignoreKeywords' in actual && !validProperties(actual.ignoreKeywords) && !validHash(actual.ignoreKeywords)) return false;

  if ('message' in actual && typeof actual.message !== 'string' && actual.message !== null) return false;
...
function validHash(actual) {
  ...
  return Object.keys(actual).every(function (key) {
    return validProperties(actual[key]);
  });
}
...
function expected(types, value, property, customMessage) {
  ...
  if (typeof customMessage === 'string' && customMessage.length) {
    ...
    return customMessage.replace('${types}', types).replace('${value}', value).replace('${property}', property);
