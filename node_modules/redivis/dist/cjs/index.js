...
async function getCachedAuthToken() {
	try {
		if (typeof window === 'undefined') {
			const { default: keytar } = await Promise.resolve().then(function () { return /*#__PURE__*/_interopNamespace(require('keytar')); });
...
async function oAuthNode() {
	const readline = await Promise.resolve().then(function () { return /*#__PURE__*/_interopNamespace(require('readline')); });
...
async function importCrypto() {
	if (!crypto) {
		if (typeof window === 'undefined') {
			const { webcrypto } = await Promise.resolve().then(function () { return /*#__PURE__*/_interopNamespace(require('crypto')); });
			crypto = webcrypto;
		} else {
...
async function importFetch() {
	if (!fetch$1) {
		if (typeof window === 'undefined') {
			const { default: nodeFetch } = await Promise.resolve().then(function () { return /*#__PURE__*/_interopNamespace(require('node-fetch')); });
			fetch$1 = nodeFetch;
		} else {
...
class Table {
	...
	async listRows(argMaxResults, options = {}) {
		...
		const { data: rows } = Papa__default['default'].parse(res);
